...
public class ShoppingcartControllers {
    ...
    //Hiển thị trang giỏ hàng cùng thông tin items, subtotal và coupon (nếu có)
    @GetMapping("/shopping-cart")
    public String shoppingCart(Model model,
                               Principal principal,
                               HttpSession session) {
        // Lấy account dựa trên tên đăng nhập
        Account account = accountService.findByUsername(principal.getName());

        //Lấy danh sách items và subtotal
        ...
        if (couponCode != null && discount != null) {
            ...
            if (subtotal < threshold) {
                //Nếu subtotal dưới ngưỡng, xóa coupon và thêm lỗi
    ...
    // Xóa mã coupon khỏi session và redirect về giỏ hàng
    ...
    //Xóa item theo productId qua Ajax, trả về JSON { productId, cartTotal }
    ...
    public Map<String,Object> removeItemAjax(@PathVariable int id, Principal principal) {
        // Lấy tài khoản đăng nhập
        Account account = accountService.findByUsername(principal.getName());
        // Xóa một mục khỏi giỏ hàng của account dựa trên productId
    ...
    //Tăng số lượng item lên 1 qua Ajax, trả về JSON { productId, quantity, lineTotal, cartTotal }
    ...
    //Giảm số lượng item đi 1 (tối thiểu 1) qua Ajax,trả về JSON { productId, quantity, lineTotal, cartTotal }
